...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    // 후기 가져오기
    let hoogiData = await fetch('../html/hoogi.json').then((response) =>
      response.json()
    );
    console.log('후기 데이터 가져옴:', hoogiData);
    loadHoogi(hoogiData);

    // card-bb를 클릭하는 이벤트 리스너 추가
    const cardBbElements = document.querySelectorAll('.card-bb img');
    cardBbElements.forEach((cardBb, index) => {
      cardBb.addEventListener('click', () => {
        // 클릭한 카드에 대한 정보
        const cardInfo = {
          picture: cardBb.querySelector('.card-img-top').src,
          like: parseInt(
            cardBb.querySelector(`#like-count-${index}`).textContent
          ),
          comment: cardBb.querySelector('.card-b-text').textContent,
  ...
  for (let i = 0; i < 12; i++) {
    let hoogi = `<div class="card text-bg-dark">
      ...
      <div class="card-img-overlay">
        ...
        <p class="card-text"><small>Last updated ${data[i].minute} mins ago</small></p>
      </div>
    </div>`;
    wrap5.innerHTML += hoogi;
  }
  const card_5 = document.q;
}
